...
const DEFAULT_STORE_PATH = join("/tmp", "onyx-waitlist-email-hashes.json");
...
function getStorePath() {
  return process.env.WAITLIST_STORAGE_PATH || DEFAULT_STORE_PATH;
}
...
async function readHashes() {
  ...
  try {
    const payload = JSON.parse(await readFile(/*turbopackIgnore: true*/ getStorePath(), "utf8")) as Partial<WaitlistStore>;
